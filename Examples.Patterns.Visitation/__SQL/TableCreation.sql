...
,	LifetimeOrderAmount	MONEY					NOT NULL	CONSTRAINT DF_Initially_Zero
															DEFAULT(0)
,	EmailValidatedOn	DATETIME				NULL
...
ADD CONSTRAINT
	CK_Need_Both_Email_And_EmailValidatedOn CHECK 
	(
		(Email IS NULL AND  EmailValidatedOn IS NULL)
	OR
		(Email IS NOT NULL AND EmailValidatedOn IS NOT NULL)
	);
GO
...
CREATE NONCLUSTERED	INDEX IX_Groceries_GroceryItem_CategoryTypeId ON Groceries.GroceryItem(CategoryTypeId);
